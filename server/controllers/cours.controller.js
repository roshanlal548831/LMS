...
export const editLecture = async (req, res) => {
    try {
        ...
        await lecture.save();
        // Ensure the course still has the lecture is if was not aleardy added;
        const course = await Course.findById(courseId);
...
export const removeLecture = async (req, res) => {
    try {
        ...
        // delete the lecture from coudinary as well
        if (lecture.publicId) {
            await deleteVideoFromCloudinary(lecture.publicId);
        }
        // remove the lecture reference from the associaed course
        await Course.updateOne(
            { lectures: lectureId },// find the course that containt the lectur
        ...
        return res.status(500).json({
            message: "Failed to remove lectur!"
        })
    }
}
